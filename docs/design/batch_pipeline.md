...
- `SUMMARY_MODEL=gpt-5.4-mini`；summary LLM model 由環境變數指定，第一版 bounded trial 已批准使用此 model。
...
- bounded trial cost estimate 使用 `.env.example` 內官方單價：embedding `0.02`、summary input `0.75`、summary output `4.50`，單位皆為 USD / 1M tokens。
...
- 允許 summary LLM calls，model 固定 `gpt-5.4-mini`；summary LLM 失敗時仍依既有規則 fallback 到 raw lines。
